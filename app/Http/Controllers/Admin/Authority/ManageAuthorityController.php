...
class ManageAuthorityController extends Controller
{
    public function viewAdmins()
    {
        try {
            $users_count = User::where('user_type', UserTypeEnum::ADMIN)->where('user_status', UserTypeEnum::ACTIVE_USER)->orderby('id', 'DESC')->get()->count();
            $users = User::where('user_type', UserTypeEnum::ADMIN)->where('user_status', UserTypeEnum::ACTIVE_USER)->orderby('id', 'DESC')->paginate(env('PAGINATION_XSMALL'));
            return view('backend.pages.admins.admin_lists')->with('users', $users)->with('users_count', $users_count);
        } catch (Exception $e) {
    ...
    public function adminUserSave(Request $request)
    {
        try {
            ...
            if (isset($_GET['edit_id']) || isset($_GET['add_id'])) {
                ...
                $data_generate .= '<div class="col-sm-6"><div class="form-group"><label>ইমেইল</label>
                                <input type="email" class="form-control" name="email"  value="' . $admin_user->email . '"></div></div>';

                $data_generate .= '<div class="col-sm-6"><div class="form-group"><label>মোবাইল নম্বর</label>
                ...
                $data_generate .= '<div class="col-sm-6"><div class="form-group"><label>ছবি</label>
                ...
                if (isset($admin_user_id)) {
                    try {
                        $validator = Validator::make($request->all(), [
                            'email' => 'nullable|email|unique:users,email,' . $admin_user_id,
    ...
    public function makeAdminUserInactive($id)
    {
        try {
    ...
    public function makeAdminUserActive($id)
    {
        try {
